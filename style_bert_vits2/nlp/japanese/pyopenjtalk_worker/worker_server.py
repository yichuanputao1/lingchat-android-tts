...
class WorkerServer:
    ...
    def handle_request(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if request_type:
            if request_type == RequestType.STATUS:
                ...
                if func_name == "unset_user_dict":
                    # unset_user_dict 函数在当前版本的 pyopenjtalk 中不存在
                    # 我们返回一个空的成功响应
    ...
    def start_server(self, port: int, no_client_timeout: int = 30) -> None:
        logger.info(f"Starting pyopenjtalk worker server on 127.0.0.1:{port}")
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
            server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            server_socket.bind(("127.0.0.1", port))
            server_socket.listen()
            logger.info(f"Worker server listening on 127.0.0.1:{port}")
